...
def process_reads(MYJOB, NJOBS, IN_READS, KMER_DICT, P_VS_Q_AMP_THRESH, return_dict):
	#
	nreads_processed   = 0
	return_dict[MYJOB] = []
	#
...
def main(raw_args=None):
	...
	parser.add_argument('--processes', type=int,   required=False, metavar='4',                 help="Number of multiprocesses to use",        default=6)
	...
	while True:
		...
		if not len(my_rdat):
			break
		all_readdat.append((my_name, my_rdat))
		nreads_read += 1
	...
	for i in range(NUM_PROCESSES):
		...
		p = multiprocessing.Process(target=process_reads, args=(i+1, NUM_PROCESSES, job_rdat, KMER_DICT, P_VS_Q_AMP_THRESH, return_dict))
		processes.append(p)
	for i in range(NUM_PROCESSES):
		processes[i].start()
	for i in range(NUM_PROCESSES):
